...
export default function PersonalLoansList() {
  ...
  const pageSize = 8;
  const [loanFilterDate, setLoanFilterDate] = useState(getTodayDate()); // Default to Today
  ...
  const fetchLoans = async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        const filtered = appsData.filter(app => app.loanType?.toLowerCase() === 'personal loan');
      ...
      console.error("Personal Loan Fetch Error:", err);
      showToast("Error fetching personal loans", "error");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const filteredApplications = useMemo(() => {
    // Convert YYYY-MM-DD to DD/MM/YYYY for comparison with dateStr
  ...
  const filteredRegistry = useMemo(() => {
    return aadhaarHistory.filter(h => {
      ...
      // verifiedAt is ISO string like 2026-04-14T...
    ...
    <div className="loans-module">
      <header className="loans-header">
        <div className="header-titles">
          <h1 className="header-title"><FileText size={28} style={{ color: '#2563eb' }} /> Personal Loan Engine</h1>
              ...
              <>
                ...
                <tbody>
                  {paginatedData.length > 0 ? (
                      ...
                      <tr key={loan.id || i}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <div className="row-actions" style={{ justifyContent: 'center', gap: '8px' }}>
                            <button className="action-icon-btn" onClick={() => navigate('/personal-loan/add', { state: { id: loan.id } })} title="Modify">
                            ...
                            <button className="action-icon-btn" onClick={() => navigate('/personal-loan-print', { state: { id: loan.id } })} title="Print" style={{ color: '#475569' }}>
                    ...
                    <tr><td colSpan="6" className="empty-state">No personal loans found for {loanFilterDate || 'selected criteria'}.</td></tr>
                  )}
                </tbody>
              </>
            ) : (
                      ...
                      <tr key={user.id || i}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <button
                            ...
                            onClick={() => navigate('/personal-loan/add', { state: { aadharData: user } })}
                            style={{ margin: '0 auto', background: '#eff6ff', color: '#1e3a8a', borderColor: '#dbeafe' }}
                          >
                            <UserPlus size={18} /> <span>Start Personal Loan</span>
                          </button>
                        </td>
                      </tr>
                    ))
