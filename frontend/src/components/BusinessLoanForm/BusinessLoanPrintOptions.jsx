...
export default function BusinessLoanPrintOptions() {
  ...
  useEffect(() => {
    ...
    if (rawData?.id && !rawData?.basic) {
      const fetchLoan = async () => {
        try {
          const res = await fetch(`${API_BASE_URL}/business-loan/${rawData.id}`, {
  ...
  const options = [
    {
      ...
      color: '#2563eb',
      desc: 'Generate and print the full business loan application form'
    },
    {
      id: 'sanction',
      ...
      color: '#10b981',
      desc: 'Generate the official sanction letter for the business'
    },
    {
      id: 'agreement',
      ...
      icon: <FileText size={32} />,
      color: '#6366f1',
      desc: 'Generate the full legal business loan agreement'
    }
  ];
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1000px', margin: '0 auto' }}>
      <header style={{ marginBottom: '32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h1 style={{ fontSize: '1.8rem', fontWeight: 800, color: '#0f172a' }}>Business Loan Print Options</h1>
        ...
        {loading ? (
          ...
          <div
            ...
            onClick={() => {
              ...
              if (opt.id === 'app') {
                navigate('/business-loan-application-print', { state: { id: loanData.id } });
              } else {
                showToast(`${opt.label} for Business Loan is currently under development.`, 'info');
              }
            }}
            style={{
