...
export default function VehicleLoanPrintOptions() {
  ...
  const [loading, setLoading] = useState(!rawData?.arjdarNaav && rawData?.id);

  useEffect(() => {
    if (rawData?.id && !rawData?.arjdarNaav) {
      const fetchLoan = async () => {
        try {
          const res = await fetch(`${API_BASE_URL}/vehicle-loan/${rawData.id}`, {
  ...
  const options = [
    {
      ...
      color: '#2563eb',
      desc: 'Generate and print the full vehicle loan application form'
    },
    {
      id: 'sanction',
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1000px', margin: '0 auto' }}>
      <header style={{ marginBottom: '32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h1 style={{ fontSize: '1.8rem', fontWeight: 800, color: '#0f172a' }}>Vehicle Loan Print Options</h1>
          <p style={{ color: '#64748b' }}>Select the specific document you wish to generate</p>
        </div>
        <button
          onClick={() => navigate('/vehicle-loan')}
          style={{
        ...
        {loading ? (
          ...
          <div
            ...
            onClick={() => {
              ...
              if (opt.id === 'app') {
                navigate(`/vehicle-loan-form-print/${loanData.id}`, { state: loanData });
              } else if (opt.id === 'sanction') {
                navigate(`/vehicle-loan-sanction-print/${loanData.id}`, { state: loanData });
              }
            }}
            style={{
